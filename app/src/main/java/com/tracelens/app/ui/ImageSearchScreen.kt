...
fun ImageSearchScreen(vm: SearchViewModel, onBack: () -> Unit, onCollections: () -> Unit, onCompare: () -> Unit) {
    ...
    TLScaffold(
        ...
        LazyColumn(
            ...
            item {
                Column {
                    SectionTitle("Mesin pencari lain")
                    Text(
                        "Fallback manual: buka mesin pencari resmi dan unggah foto sendiri. TraceLens tidak melakukan scraping atau melewati login/CAPTCHA.",
            ...
            item {
                SectionTitle("Web publik · privacy-first")
                Text(
                    "Tanpa API key, proxy, database, atau server TraceLens. Gambar dikirim langsung dari perangkat ke mesin yang lu pilih; TraceLens tidak menyimpan salinannya.",
                    style = MaterialTheme.typography.bodySmall,
                )
            }
            ...
            item {
                Text(
                    "Catatan: pencarian web memang harus mengirim gambar ke indeks publik. Yang dihindari di sini adalah perantara/penyimpanan pihak ketiga milik TraceLens.",
