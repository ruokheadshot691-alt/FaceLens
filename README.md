...
## Public Web Image Search

Image Search sekarang punya workflow **Web publik** berbasis Google Cloud Vision Web Detection. Setelah memilih foto, tekan **Cari di web publik** untuk mendapatkan URL gambar/halaman yang terdeteksi sebagai exact match, partial/crop match, atau halaman yang memuat gambar tersebut.

Google Cloud Vision menyediakan `WEB_DETECTION` yang dapat mengembalikan matching images dan pages with matching images dari Internet.

### Build configuration

Tambahkan API key ke `gradle.properties` lokal (jangan commit ke Git):
...
> Untuk produksi, jangan menanam API key permanen di APK. Gunakan backend/proxy yang memegang secret dan batasi quota/domain. TraceLens tidak melakukan scraping, bypass login/CAPTCHA, atau mengakses akun privat.
